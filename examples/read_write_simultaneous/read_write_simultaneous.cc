...
void print_feedback(const std::optional<robotiq::GripperFeedback> & feedback) {
  if (!feedback) {
    std::cout << "Feedback is empty, something went wrong!\n";
    return;
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Move to 60% with no force\n";
  print_feedback(gripper.set_and_read_gripper(60.0, 50.0, 100));
  std::cout << "\n\nPress Enter to continue...\n\n" << std::endl;
  std::cin.get();

  // Hold the position but change the force. Grip at 50%
  std::cout << "Grip 50% tighter\n";
  print_feedback(gripper.set_and_read_gripper(61.0, 50.0, 50));
  std::cout << "\n\nPress Enter to continue...\n\n" << std::endl;
  std::cin.get();

  // Hold the position but change the force. Grip tighter at 100%
  std::cout << "Grip tightly\n";
  print_feedback(gripper.set_and_read_gripper(62.0, 50.0, 0.0));
  std::cout << "\n\nPress Enter to continue...\n\n" << std::endl;
  std::cin.get();

  // Hold the position but change the force. Relax it to 0%
  std::cout << "Relax grip\n";
  print_feedback(gripper.set_and_read_gripper(61.0, 50.0, 100.0));
  std::cout << "\n\nPress Enter to continue...\n\n" << std::endl;
  std::cin.get();
